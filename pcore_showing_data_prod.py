...
try:
    ...
    cur_chartio.execute("TRUNCATE TABLE " + 'pcore_showing_data')
    #con_chartio.commit()
    cur_chartio.execute("DROP TABLE IF EXISTS " + 'pcore_showing_data')
    #con_chartio.commit()
    cur_chartio.execute("CREATE TABLE " + 'pcore_showing_data' +'()')
    con_chartio.commit()

    for column in data2.columns:
        cur_chartio.execute("ALTER TABLE " + 'pcore_showing_data' +" ADD COLUMN " + column.lower() + " text;")
        con_chartio.commit()

    columnString = ','.join(data2.columns)

    cur_chartio.copy_from(open('pcore_showing_data'+'output.csv','r'), 'pcore_showing_data', sep=';', null='NA', columns=None)
    con_chartio.commit()
  
    cur_chartio.execute("GRANT ALL ON TABLE " + 'pcore_showing_data' +" TO GROUP reporting_role;")
    con_chartio.commit()
    ...
    sender = cf.gmail_id
    receivers = [cf.recip]
    message = """From: Linux Box <{0}>\nTo:<{1}>\nSubject: ERROR LOG-FAIL TO WRITE {2}\n\nError message: {3}.""".format(cf.gmail_id,cf.recip,'pcore_showing', error_log)
    try:
